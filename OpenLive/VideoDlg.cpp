...
{
	int TOP_MARGIN = m_rcVideoArea.Height() / 5;

	m_wndBigVideo.MoveWindow(m_rcVideoArea.Width() / 4, m_rcVideoArea.top + TOP_MARGIN, m_rcVideoArea.Width() * 3 / 4, m_rcVideoArea.Height() * 4 / 5, FALSE);
	m_wndLocal.MoveWindow(0, m_rcVideoArea.top + TOP_MARGIN, m_rcVideoArea.Width() / 4, m_rcVideoArea.Height() / 4, FALSE);
	m_wndSecond.MoveWindow(0, m_rcVideoArea.top + TOP_MARGIN + m_rcVideoArea.Height() / 4, m_rcVideoArea.Width() / 4, m_rcVideoArea.Height() / 4, FALSE);
}
...
void CVideoDlg::OnBnClickedBtnclose()
{
	...
	m_wndBigVideo.SetUID(0);
	m_wndSecond.SetUID(0);
...
{
	BOOL bShowed = m_wndLocal.IsVideoInfoShowed();

	m_wndLocal.ShowVideoInfo(!bShowed);
	m_wndBigVideo.ShowVideoInfo(!bShowed);
	m_wndSecond.ShowVideoInfo(!bShowed);

	if (m_wndLocal.IsVideoInfoShowed())
...
void CVideoDlg::SwitchBetweenSmallBigViews()
{
	if (m_wndLocal.GetSafeHwnd() == NULL || m_wndBigVideo.GetUID() == 0 || m_wndSecond.GetUID() == 0)
		return;
	...
	while(pos != NULL){
		...
		if (m_wndBigVideo.GetUID() != agvWndInfo.nUID && agvWndInfo.nIndex == 2) {
			canvas.view = m_wndBigVideo.GetSafeHwnd();

			m_wndBigVideo.SetUID(canvas.uid);
			m_wndBigVideo.SetVideoResolution(agvWndInfo.nWidth, agvWndInfo.nHeight);
			m_wndBigVideo.SetFrameRateInfo(agvWndInfo.nFramerate);
			m_wndBigVideo.SetBitrateInfo(agvWndInfo.nBitrate);

			agvWndInfo.nIndex = 1; // big

			CAgoraObject::GetEngine()->setupRemoteVideo(canvas);
		}
		else if (m_wndSecond.GetUID() != agvWndInfo.nUID && agvWndInfo.nIndex == 1) {
			canvas.view = m_wndSecond.GetSafeHwnd();

			m_wndSecond.SetUID(canvas.uid);
			m_wndSecond.SetVideoResolution(agvWndInfo.nWidth, agvWndInfo.nHeight);
			m_wndSecond.SetFrameRateInfo(agvWndInfo.nFramerate);
			m_wndSecond.SetBitrateInfo(agvWndInfo.nBitrate);
...
void CVideoDlg::OnBnClickedBtnmode()
{
	...
	// switch video view between small and big

	SwitchBetweenSmallBigViews();
...
LRESULT CVideoDlg::OnEIDUserOffline(WPARAM wParam, LPARAM lParam)
{
	...
	while (pos != NULL){
		if (m_listWndInfo.GetAt(pos).nUID == lpData->uid) {
			...
			if (m_listWndInfo.GetAt(pos).nIndex == 1) {
				m_wndBigVideo.SetUID(0);
			}
			if (m_listWndInfo.GetAt(pos).nIndex == 2) {
				m_wndSecond.SetUID(0);
			}
...
LRESULT CVideoDlg::OnRemoteVideoStat(WPARAM wParam, LPARAM lParam)
{
	...
	while (posNext != NULL) {
		...
		if (rWndInfo.nUID == lpData->uid) {
			...
			if (rWndInfo.nIndex == 0) {
				m_wndLocal.SetFrameRateInfo(rWndInfo.nFramerate);
...
void CVideoDlg::DrawHead(CDC *pDC)
{
	...
	pDC->SelectObject(&m_ftDes1);
	pDC->DrawText(L"Ö±²¥½ÌÊÒ Live Class Room", CRect(180, 50, 850, 300), DT_SINGLELINE);
...
void CVideoDlg::InitCtrls()
{
	...
	m_btnShow.MoveWindow(rcClient.Width() / 2 + 144, rcClient.Height() - 84, 72, 72, TRUE); // m_btnShow.MoveWindow(8, 36, 72, 72, TRUE);
	...
	m_wndBigVideo.MoveWindow(0, 24, rcClient.Width(), rcClient.Height() - 96, TRUE);
...
void CVideoDlg::ShowVideo4()
{
	...
	m_wndSecond.ShowWindow(SW_SHOW);
	m_wndSecond.SetBigShowFlag(FALSE);
	m_wndSecond.SetParent(this);

	m_wndBigVideo.ShowWindow(SW_SHOW);
	m_wndBigVideo.SetBigShowFlag(TRUE);
	m_wndBigVideo.SetParent(this);
...
BOOL CVideoDlg::OnInitDialog()
{
	...
	m_ftDes1.CreateFont(60, 0, 0, 0, FW_EXTRABOLD | FW_HEAVY, FALSE, FALSE, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS , _T("Arial"));
...
void CVideoDlg::bindVideoWnd1()
{
	...
	while (pos != NULL) {
		...
		if (agvWndInfo.nUID == 0 || agvWndInfo.nUID == lpAgoraObject->GetSelfUID()) {
			break; // already binded
		}

		canvas.uid = agvWndInfo.nUID;

		if (m_wndBigVideo.GetUID() == 0 && agvWndInfo.nIndex == 0) {
			canvas.view = m_wndBigVideo.GetSafeHwnd();
		...
		else if (m_wndSecond.GetUID() == 0 && agvWndInfo.nIndex == 0) {
			canvas.view = m_wndSecond.GetSafeHwnd();
...
void CVideoDlg::RebindVideoWnd()
{
	...
	while (pos != NULL) {
		...
		CAgoraObject *lpAgoraObject = CAgoraObject::GetAgoraObject();
		if (agvWndInfo.nUID == lpAgoraObject->GetSelfUID()) {
			canvas.view = m_wndLocal.GetSafeHwnd();

			m_wndLocal.SetUID(canvas.uid);
			m_wndLocal.SetVideoResolution(agvWndInfo.nWidth, agvWndInfo.nHeight);
			m_wndLocal.SetFrameRateInfo(agvWndInfo.nFramerate);
			m_wndLocal.SetBitrateInfo(agvWndInfo.nBitrate);
