...
LRESULT COpenLiveDlg::OnJoinChannel(WPARAM wParam, LPARAM lParam)
{
	...
	AParameter apm(*lpRtcEngine);

	nRet = apm->setParameters("{\"che.video.moreFecSchemeEnable\":true}");
	nRet = apm->setParameters("{\"che.video.fecMethod\":2}");

	if (m_dlgEnterChannel.IsIntraRequestEnabled()) {
		nRet = apm->setParameters("{\"che.video.has_intra_request\":true}");
	}

	if (m_dlgEnterChannel.IsUCEnabled()) {
//		nRet = apm->setParameters("{\"rtc.force_unified_communication_mode\":true}");
	}

	int nEncodeType = m_dlgEnterChannel.getEncodeType();
	char szbuf[MAX_PATH] = { '\0' };
	sprintf(szbuf,"{\"che.hardware_encoding\":%d}",nEncodeType);
	nRet = apm->setParameters(szbuf);
...
//	lpRtcEngine->setVideoProfile((VIDEO_PROFILE_TYPE)nVideoSolution, m_dlgSetup.IsWHSwap());
	...
	nRet = apm->setParameters("{\"che.video.freestyle_customer\":true}");

	VideoEncoderConfiguration vconfig(1920, 1080, FRAME_RATE::FRAME_RATE_FPS_30, 2048, ORIENTATION_MODE::ORIENTATION_MODE_FIXED_LANDSCAPE);
	lpRtcEngine->setVideoEncoderConfiguration(vconfig);
