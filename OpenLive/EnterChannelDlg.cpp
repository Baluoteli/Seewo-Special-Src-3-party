...
void CEnterChannelDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CKINTRAREQUEST, m_ckIntraRequest);
    DDX_Control(pDX, IDC_CKUC, m_ckUC);
	...
	DDX_Control(pDX, IDC_COMBO_ENCODETYPE, m_comEncodeType);
}
...
{
	m_ckIntraRequest.MoveWindow(80, 180, 120, 30, TRUE);
	m_ckIntraRequest.SetCheck(TRUE);

	m_ckUC.MoveWindow(80, 220, 120, 30, TRUE);
	m_ckUC.SetCheck(TRUE);

	m_comEncodeType.MoveWindow(80, 260, 120, 30, TRUE);
	m_comEncodeType.AddString(L"SOFTWARE");
	m_comEncodeType.AddString(L"NVIDIA_HARDWARE");
	m_comEncodeType.AddString(L"INTEL_NOSPECIAL");
	m_comEncodeType.AddString(L"NOT_SPECIAL");
	m_comEncodeType.SetCurSel(0);
...
BOOL CEnterChannelDlg::IsIntraRequestEnabled()
{
	return m_ckIntraRequest.GetCheck();
}

BOOL CEnterChannelDlg::IsUCEnabled()
{
	return m_ckUC.GetCheck();
}

int	 CEnterChannelDlg::getEncodeType()
{
	int nCurSel = m_comEncodeType.GetCurSel();
